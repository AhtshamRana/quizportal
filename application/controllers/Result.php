...
class Result extends CI_Controller {
	...
	if($val=='0'){
	 ...
	 $data['percentile'] = $this->result_model->get_percentile($data['result']['quid'], $data['result']['uid'], $data['result']['score_obtained']);
	  ...
	  $query=$this->db->query(" select * from portal_result where score_obtained > '$score' and quid ='$quid' group by score_obtained ");
	  $data['rank']=$query->num_rows() + 1;
	  $query=$this->db->query(" select * from portal_result where quid ='$quid'  group by score_obtained  ");
	  $data['last_rank']=$query->num_rows();
	  $query=$this->db->query(" select * from portal_result where quid ='$quid'  group by score_obtained  order by score_obtained desc limit 3 ");
	  $data['toppers']=$query->result_array();
	  $query=$this->db->query(" select * from portal_result where quid ='$quid'  group by score_obtained  order by score_obtained asc limit 1 ");
