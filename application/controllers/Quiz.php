...
function open_quiz($limit='0'){
	...
	function no_q_available($cid,$lid){
		$val="<select name='noq[]'>";
		$query=$this->db->query(" select * from portal_qbank where cid='$cid' and lid='$lid' ");
		$nor=$query->num_rows();
		for($i=0; $i<= $nor; $i++){
	...
	public function validate_quiz($quid){
		...
		$this->db->insert('portal_users',$userdata);
		$uid=$this->db->insert_id();
		$query=$this->db->query("select * from portal_users where uid='$uid' ");
