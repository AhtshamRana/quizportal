...
Class Qbank_model extends CI_Model
{
  ...
  function question_list($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('portal_qbank.qid',$search);
		 $this->db->or_like('portal_qbank.question',$search);
		 $this->db->or_like('portal_qbank.description',$search);
	 ...
	 if($lid!='0'){
		 ...
		 $this->db->join('portal_category','portal_category.cid=portal_qbank.cid');
	 $this->db->join('portal_level','portal_level.lid=portal_qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('portal_qbank.qid','desc');
		$query=$this->db->get('portal_qbank');
 ...
 function get_option($qid){
	 $this->db->where('qid',$qid);
	 $query=$this->db->get('portal_options');
	 return $query->result_array();
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('portal_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('portal_options');
		 return true;
	 }else{
 ...
 function insert_question_1(){
	 ...
	 $this->db->insert('portal_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('portal_options',$userdata);	 
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('portal_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('portal_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function update_category($cid){
		...
		if($this->db->update('portal_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('portal_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('portal_category',$userdata)){
...
// level function start
 function level_list(){
	  $query=$this->db->get('portal_level');
	 return $query->result_array();
 ...
 function update_level($lid){
		...
		if($this->db->update('portal_level',$userdata)){
 ...
 function remove_level($lid){
	 ...
	 $this->db->where('lid',$lid);
	 if($this->db->delete('portal_level')){
		 return true;
	 }else{
 ...
 function insert_level(){
		...
		if($this->db->insert('portal_level',$userdata)){
...
$question_type=$this->lang->line('long_answer');	
	...
	if($this->db->insert('portal_qbank',$insert_data)){
		$qid=$this->db->insert_id();
		$optionkeycounter = 4;
		if($ques_type=="0" || $ques_type==""){
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("portal_options",$insert_options);
				$optionkeycounter++;
				}
